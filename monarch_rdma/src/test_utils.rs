...
impl RdmaManagerTestEnv<'_> {
    ...
    pub async fn setup(
        ...
        let mut device_str1 = (String::new(), 0);
        let mut device_str2 = (String::new(), 0);
    ...
    pub async fn cleanup(self) -> Result<(), anyhow::Error> {
        self.actor_1
            .release_buffer(self.client_1, self.rdma_handle_1)
            .await?;
        self.actor_2
            .release_buffer(self.client_2, self.rdma_handle_2)
            .await?;

...
impl RdmaManagerMessageHandler for RdmaManagerActor {
    ...
    async fn initialize_qp(
        ...
        if let std::collections::hash_map::Entry::Vacant(e) = self.qp_map.entry(key) {
            println!("initializing connection with {:?}", other);
    ...
    async fn connection_info(
        ...
        // if !self.qp_map.contains_key(&other.actor_id().clone()) {
        //     println!("triggering local qp init");
        ...
        println!(
            "actor {} remote endpoint: {:?}",
            other.actor_id().clone(),
            connection_info
        );
        println!("trying to return info");
        Ok(connection_info)
    }
}
...
mod tests {
    ...
    async fn test_rdma_read_loopback() -> Result<(), anyhow::Error> {
        ...
        let mut env = RdmaManagerTestEnv::setup(BSIZE, Some((0, 0))).await?;
        let mut qp_1 = env
            .actor_1

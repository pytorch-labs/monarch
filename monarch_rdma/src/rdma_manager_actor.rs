...
impl RdmaManagerMessageHandler for RdmaManagerActor {
    async fn register_local_memory(
        ...
        {
            tracing::error!("raising error");
            return Err(anyhow::anyhow!(
                "memory region {:?} is already registered",
                local_memory_region
            ));
        }
...
mod tests {
    ...
    impl RdmaManagerTestEnv {
        ...
        async fn setup(
            ...
            let _ = tracing_subscriber::fmt()
                .with_env_filter("monarch_rdma=debug")
                .try_init();
            ...
            println!("all meshes are spawned");
    ...
    async fn test_rdma_read_separate_devices() -> Result<(), anyhow::Error> {
        ...
        println!("using remote buffer: {:?}", remote_buffer);
        let work_id = env
            .actor_2
            .fetch(
                client,

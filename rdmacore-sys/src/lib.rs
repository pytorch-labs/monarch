...
// NOTE: This is a copy of `lib.rs` from https://fburl.com/8lo1m3il. ibverbs-sys
// is primarily just auto-generated bindings from bindgen, *except* for
// `ibv_wc`. We could vendor the upstream ibverbs-sys directly with Reindeer,
// but we want to link against our internal rdma-core version, and Reindeer
// doesn't support providing cpp_library headers as inputs to the environment
// of the buildscript (e.g., to find <infiniband/verbs.h>). So, we just have a
// dedicated rust_bindgen_library and have this file copied. That means we also
// need to keep this file in-sync w/ the upstream version as well.

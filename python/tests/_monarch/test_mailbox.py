...
class Reducer(Generic[U]):
    ...
    def __call__(self, left: PythonMessage, right: PythonMessage) -> PythonMessage:
        ...
        return PythonMessage(left.method, pickle.dumps(result))
...
class Accumulator(Generic[S, U]):
    ...
    def __call__(self, state: PythonMessage, update: PythonMessage) -> PythonMessage:
        ...
        return PythonMessage(state.method, pickle.dumps(result))

    @property
    def initial_state(self) -> PythonMessage:
        return PythonMessage(
            " @Accumulator.initial_state", pickle.dumps(self._initial_state)
        )
...
async def test_accumulator() -> None:
    ...
    def post_message(value: int) -> None:
        mailbox.post(
            port_id,
            PythonMessage("test_accumulator", pickle.dumps(value)),
        )
...
class MyActor:
    ...
    async def handle_cast(
        ...
        mailbox.post(reply_port, PythonMessage("echo", message.message))
        for i in range(100):
            mailbox.post(reply_port, PythonMessage("echo", pickle.dumps(f"msg{i}")))
...
async def test_reducer() -> None:
    ...
    actor_mesh.cast(PythonMessage("echo", pickle.dumps("start"), port_id))

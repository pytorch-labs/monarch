    ...
    try:
        ...
        with tempfile.NamedTemporaryFile(mode="w+", delete=False) as stdout_file:
            ...
            try:
                ...
                # Set a long aggregation window to ensure logs aren't flushed by time
                await pm.logging_option(stream_to_client=True, aggregate_window_sec=60)

                # Generate some logs that will be aggregated but not flushed by time
                for _ in range(5):
                ...
                # Wait a bit to ensure logs are collected but not flushed by time
                await asyncio.sleep(1)
                for _ in range(5):
        ...
        assert re.search(
            r"similar log lines.*aggregated log line", stdout_content
        ), stdout_content

        # The logs should appear in the aggregated format (with count)
        assert re.search(
            r"\[5 similar log lines\].*aggregated log line", stdout_content
        ), stdout_content
        ...
        assert re.search(r"single log line", stdout_content), stdout_content
...
async def test_adjust_aggregation_window() -> None:
    ...
    try:
        ...
        assert re.search(
            r"similar log lines.*first batch of logs", stdout_content
        ), stdout_content
        ...
        # The logs should appear in the aggregated format with the correct counts
        assert re.search(

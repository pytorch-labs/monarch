...
async def test_actor_log_streaming() -> None:
    ...
    try:
        ...
        # Has a leading space so we can distinguish between streamed log and
        # the log directly printed by the child processes as they share the same stdout/stderr
        assert " no print streaming" not in stdout_content, stdout_content
        assert " no print streaming" not in stderr_content, stderr_content
        assert " no log streaming" not in stdout_content, stdout_content
        assert " no log streaming" not in stderr_content, stderr_content
        assert (
            "no log streaming due to level mismatch" not in stdout_content
        ), stdout_content
        assert (
            "no log streaming due to level mismatch" not in stderr_content
        ), stderr_content

        assert " has print streaming" in stdout_content, stdout_content
        assert " has print streaming" not in stderr_content, stderr_content
        assert " has print streaming too" in stdout_content, stdout_content
        assert " has print streaming too" not in stderr_content, stderr_content
        assert (
            " has log streaming as level matched" not in stdout_content
        ), stdout_content
        assert " has log streaming as level matched" in stderr_content, stderr_content

...
pub struct PythonActorMesh {
    ...
    unhealthy_event: Arc<std::sync::Mutex<Option<ActorSupervisionEvent>>>,
    user_monitor_sender: tokio::sync::broadcast::Sender<ActorSupervisionEvent>,
...
impl PythonActorMesh {
    ...
    pub(crate) fn monitored(
        ...
        let (user_monitor_sender, _) = tokio::sync::broadcast::channel::<ActorSupervisionEvent>(1);
    ...
    async fn actor_mesh_monitor(
        ...
        user_sender: tokio::sync::broadcast::Sender<ActorSupervisionEvent>,
        unhealthy_event: Arc<std::sync::Mutex<Option<ActorSupervisionEvent>>>,
    ) {
        loop {
...
impl PythonActorMesh {
    ...
    fn get_supervision_event(&self) -> PyResult<Option<PyActorSupervisionEvent>> {
        ...
        Ok(unhealthy_event
            .as_ref()
            .map(|event| PyActorSupervisionEvent::from(event.clone())))
    }
...
pub struct PyActorMeshMonitor {
    receiver: SharedCell<Mutex<tokio::sync::broadcast::Receiver<ActorSupervisionEvent>>>,
}
    ...
    pub fn __anext__(&self, py: Python<'_>) -> PyResult<PyObject> {
        ...
        Ok(
            pyo3_async_runtimes::tokio::future_into_py(
                py,
                async move { get_next(receiver).await },
            )?
            .into(),
        )
    }
...
async fn get_next(
    ...
    let receiver = receiver
        .borrow()
        .map_err(|_| PyRuntimeError::new_err("`Actor mesh receiver` is shutdown"))?;
    let mut receiver = receiver.lock().await;
    let event = receiver.recv().await.unwrap();

    Ok(Python::with_gil(|py| {
        PyActorSupervisionEvent::from(event).into_py(py)
    }))
}

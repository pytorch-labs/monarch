        ...
        for (path, (origin, metadata)) in self.state.into_iter() {
            match origin {
                ...
                Origin::Dst => {
                    actions.insert(
                        path,
                        Action::Delete(matches!(metadata.ftype, FileTypeInfo::Directory)),
                    );
                }
            }
        }
...
pub async fn receiver(
    ...
    for (path, action) in actions.iter() {
        ...
        match action {
            ...
            Action::Delete(is_dir) => {
                deletions.insert(path, *is_dir);
            }
        }
    }
...
mod tests {
    ...
    async fn test_sync_modified_file() -> Result<()> {
        ...
        let expected_actions = HashMap::from([(
            ...
            Action::Receive(newer_time, Receive::File(false)),
        )]);
    ...
    async fn test_sync_file_deletion() -> Result<()> {
        ...
        let expected_actions =
            HashMap::from([(PathBuf::from("extra-file.txt"), Action::Delete(false))]);
    ...
    async fn test_sync_ignores_pyc_files() -> Result<()> {
        ...
        let expected_actions = HashMap::from([
            (PathBuf::from("lib/__pycache__"), Action::Delete(true)),
            (
    ...
    async fn test_sync_executable_permissions() -> Result<()> {
        ...
        let expected_actions = HashMap::from([(
            ...
            Action::Receive(newer_time, Receive::File(true)),
        )]);

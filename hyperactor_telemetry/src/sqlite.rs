...
lazy_static! {
    static ref ACTOR_LIFECYCLE: Table = (
        ...
        .into();
    static ref ACTORS: Table = ("actors", ["actor_id", "name", "module_path"].as_slice()).into();
...
impl SqliteLayer {
    ...
    fn insert_event(&self, event: &Event<'_>) -> Result<()> {
        ...
        match (event.metadata().target(), event.metadata().name()) {
            ("message", _) => {
                insert_event!(MESSAGES, conn, event);
            }
            ("actor_lifecycle", _) => {
                insert_event!(ACTOR_LIFECYCLE, conn, event);
            }
            _ => {
                insert_event!(EVENTS, conn, event);
            }
        }
        Ok(())
    }
...
fn print_table(conn: &Connection, table_name: &str) -> Result<()> {
    // Get column names
    ...
    // Print header
    println!("=== {} ===", table_name.to_uppercase());
    ...
    let rows = stmt.query_map([], |row| {
        ...
        for i in 0..columns.len() {
            ...
            let value = if i == 0 && columns[i] == "seq" {
...
mod tests {
    ...
    fn test_sqlite_layer() -> Result<()> {
        ...
        info!(target:"message", test_field = "test_value", "Test event");

        let count: i64 =
            conn.lock()
                .unwrap()
                .query_row("SELECT COUNT(*) FROM messages", [], |row| row.get(0))?;
        print_table(&conn.lock().unwrap(), "messages")?;
        assert!(count > 0);
        Ok(())
    }
}

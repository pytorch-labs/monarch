...
///         let mut bindings_iter = BindingsIter::from(bindings);
///         *self.field2.port_id_mut() =
///             bindings_iter
///                 .next_t::<hyperactor::PortId>()
///                 .ok_or_else(|| anyhow::anyhow!("no value found"))?;
///         *self.field4.port_id_mut() =
///             bindings_iter
///                 .next_t::<hyperactor::PortId>()
///                 .ok_or_else(|| anyhow::anyhow!("no value found"))?;
///         Ok(self)
...
pub fn derive_bind(input: TokenStream) -> TokenStream {
    fn make_item(field_accessor: proc_macro2::TokenStream, _ty: Type) -> proc_macro2::TokenStream {
        quote! {  *#field_accessor.port_id_mut() = bindings_iter.next_t::<hyperactor::PortId>().ok_or_else(|| anyhow::anyhow!("no value found"))?; }
    }
    ...
    let expand = quote! {
        impl hyperactor::message::Bind for #name {
            fn bind(mut self, bindings: &hyperactor::message::Bindings) -> anyhow::Result<Self> {
                let mut bindings_iter = hyperactor::message::BindingsIter::from(bindings);
                #inner
                Ok(self)
            }
        }

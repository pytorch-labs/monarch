...
mod tests {
    ...
    mod process {
        ...
        async fn test_fail_sending_to_comm_actor_during_cast() {
            let max_frame_length = 1_000_000;
            ...
            // Wait for proc mesh and actor mesh to be fully up. Not wait and
            // execute the failing cast too soon could cause channel starvation
            // since its run loop is busy trying to send the big message. That
            // would make other messages sent during the mesh startup failed to
            // be acked.
